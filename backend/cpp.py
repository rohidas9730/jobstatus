...
class Job_Status(db.Model):
    id = db.Column(db.Integer, db.ForeignKey('Job.id'))
...
def home():
    if request.method == 'POST':
        ...
        job = Job(id=option_id,name=file_name,date=dt,start_time = human_readable_st,end_time = human_readable_en,duration=dur,status = stat)
        db.session.add(job)
        db.session.commit()
        # result = db.session.execute(db.select(Job_Status).order_by(desc(Job_Status.end_time)))
        all_jobs= db.session.query(Job_Status).order_by(desc(Job_Status.end_time)).all()
